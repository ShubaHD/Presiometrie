...
app.use((req, res, next) => {
  ...
  res.setHeader(
    ...
    "Content-Type, x-report-secret, x-report-token, x-report-token-exp",
  );
...
function supabaseProjectUrlAndKey(): { url: string; key: string } {
  ...
  try {
    ...
    if (e instanceof Error && e.message.startsWith("SUPABASE_URL")) throw e;
    throw new Error(
      "SUPABASE_URL invalid în variabilele de mediu (Railway). Copiați Project URL din Supabase → Settings → API (https://….supabase.co), fără spații sau ghilimele.",
    );
  }
  return { url, key };
}
...
function requireSecretOrToken(req: express.Request, res: express.Response, testId: string): boolean {
  ...
  if (hdrSecret === shared) return true;
...
function normalizeTestTypeForReport(raw: unknown): string {
  if (raw == null) return "";
  return String(raw)
    .trim()
    .toLowerCase()
...
function sanitizeStorageFileSegment(raw: string): string {
  const s = String(raw ?? "")
    .trim()
    .replace(/[\u200b-\u200d\ufeff]/g, "");
  if (!s) return "fara-cod-proba";
  let out = s
    .replace(/[/\\:*?"<>|]+/g, "-")
...
async function buildHtmlForTest(
  ...
  if (tErr) throw tErr;
  const testTypeRaw = (tRow as { test_type?: unknown }).test_type;
