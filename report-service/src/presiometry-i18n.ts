/**
 * Texte statice raport presiometrie RO/EN (PDF).
 * Rândurile din măsurători/rezultate (label din DB) se traduc prin `pmtTableLabelForLocale`.
 */
export type ReportLocale = "ro" | "en";
...
const EN: Record<string, string> = {
  ...
  specimen_note:
    "Report §8.1 k: description / sketch / photo of the specimen showing the failure type (where applicable).",
...
const RO: Record<string, string> = {
  ...
  specimen_note:
    "Raport §8.1 k: descriere / schiță / fotografie a probei care indică tipul de rupere.",
  specimen_before: "Înainte de încercare",
  specimen_after: "După încercare",
};

export function presiometryStaticCopy(locale: ReportLocale): Record<string, string> {
  return locale === "en" ? { ...EN } : { ...RO };
}

export function formatTestDateForReport(raw: string | null, locale: ReportLocale): string {
...
function programEn(tt: string): string {
  if (tt === "presiometry_program_a") return EN.program_a;
  if (tt === "presiometry_program_b") return EN.program_b;
  return EN.program_c;
}

function programRo(tt: string): string {
  if (tt === "presiometry_program_a") return RO.program_a;
  if (tt === "presiometry_program_b") return RO.program_b;
  return RO.program_c;
}

export function presiometryReportMainTitle(testType: string, locale: ReportLocale): string {
  const p = locale === "en" ? programEn(testType) : programRo(testType);
  if (locale === "en") return `${EN.report_title} — ${p}`;
  return `${RO.report_title} — ${p}`;
}

export function presiometryPageTitle(testType: string, locale: ReportLocale): string {
  const p = locale === "en" ? programEn(testType) : programRo(testType);
  if (locale === "en") return `Pressuremeter (${p})`;
 ...
 */
export function pmtTableLabelForLocale(key: string, labelRo: string, locale: ReportLocale): string {
  if (locale === "ro") return labelRo;
  const k = String(key);
  ...
  if (/_gur_r2$/.test(k)) return "GUR: R² — linear fit quality (loop)";
  if (/_gur_n$/.test(k)) return "GUR: N points — points in GUR regression (loop)";
  if (/_unload_r2$/.test(k)) return labelRo.replace(/^GU(\d+): R²/, "GU$1: R² — fit on unloading (loop $1)");
  if (/_reload_r2$/.test(k)) return labelRo.replace(/^GR(\d+): R²/, "GR$1: R² — fit on reloading (loop $1)");
  return labelRo;
}

export function pmtMeasurementLabelForLocale(key: string, labelRo: string, locale: ReportLocale): string {
  if (locale === "ro") return labelRo;
  const k = String(key);
  if (k === "pmt_series_axis") {
    if (labelRo.includes("p–V") || labelRo.includes("p-V")) return EN.series_axis_pv;
    return EN.series_axis_pr;
  }
  if (k === "pmt_start_time") return EN.start_time_csv;
  if (k === "pmt_packer_diameter_mm") return EN.packer_diameter;
  if (k === "pmt_probe_type") return "Probe / pressuremeter type (optional)";
  if (k === "pmt_seating_r_mm") return "Seating R (mm)";
  if (k === "pmt_borehole_diameter_mm") return "Borehole diameter (optional)";
  if (k === "pmt_initial_volume_cm3") return "Initial volume V₀ (optional)";
  if (k === "pmt_temperature_c") return "Temperature (optional)";
  if (k === "pmt_notes_field") return "Field notes (optional)";
  if (k === "pmt_depth_m") return "Test depth z";
  return labelRo;
}

export function parseReportLocale(v: unknown): ReportLocale {
