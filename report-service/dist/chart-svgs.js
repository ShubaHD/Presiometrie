...
const SVG_FONT = 'font-family="Arial, Helvetica, DejaVu Sans, Liberation Sans, sans-serif"';
...
function axisRange(min, max, padRatio = 0.06) {
    ...
    if (hi <= 0 && max >= 0)
        hi = max + span * 0.05;
    if (lo > 0 && min >= 0)
        lo = Math.max(0, min - span * 0.02);
    return { lo, hi };
}
function axisDomainFromZero(min, max, padRatio = 0.06) {
    if (!Number.isFinite(min) || !Number.isFinite(max))
        return { lo: 0, hi: 1 };
    if (min >= 0) {
...
function buildTickValues(lo, hi) {
    const span = hi - lo;
    ...
    if (ticks[0] > lo + span * 0.03)
        ticks.unshift(lo);
    const last = ticks[ticks.length - 1];
    if (last < hi - span * 0.03)
        ticks.push(hi);
    return ticks;
}
function fmtTickTime(v) {
    if (!Number.isFinite(v))
        return "—";
    const a = Math.abs(v);
    if (a >= 1000 || (a > 0 && a < 1e-6))
...
function fmtTickStrain(v) {
    if (!Number.isFinite(v))
        return "—";
    const a = Math.abs(v);
    if (a >= 1000 || (a > 0 && a < 1e-5))
        return v.toExponential(1);
    if (a < 0.1)
        return v.toFixed(4);
    return v.toFixed(3);
}
function fmtTickLoad(v) {
    if (!Number.isFinite(v))
        return "—";
    const a = Math.abs(v);
    if (a >= 1000 || (a > 0 && a < 1e-4))
        return v.toExponential(1);
    if (a < 10)
        return v.toFixed(2);
    if (a < 100)
...
function svgAxesAndTicks(minX, maxX, minY, maxY, fmtX, fmtY) {
    const innerW = W - PAD_L - PAD_R;
    const innerH = H - PAD_T - PAD_B;
    const sx = innerW / (maxX - minX || 1e-12);
    const sy = innerH / (maxY - minY || 1e-12);
    const X = (x) => PAD_L + (x - minX) * sx;
    const Y = (y) => PAD_T + (maxY - y) * sy;
    ...
    const rangeLine = `<text x="${cx}" y="${H - 20}" text-anchor="middle" font-size="8" fill="#333" ${SVG_FONT}>${escXml(`Plajă: ${fmtX(minX)} … ${fmtX(maxX)} (abscisă) · ${fmtY(minY)} … ${fmtY(maxY)} (ordonată)`)}</text>`;
...
export function stressStrainSvg(points, title = "σ – ε_axial") {
    if (points.length < 2)
        return null;
    ...
    const { grid, xLbl, yLbl, rangeLine, X, Y, innerH } = svgAxesAndTicks(rx.lo, rx.hi, ry.lo, ry.hi, fmtTickStrain, fmtTickStress);
...
export function loadStrainSvg(points, title = "Sarcină – ε_axial", yLabel = "F (kN)") {
    if (points.length < 2)
        return null;
    const dec = decimate(points, 800);
    const xs = dec.map((p) => p.strain);
    ...
    const { grid, xLbl, yLbl, rangeLine, X, Y, innerH } = svgAxesAndTicks(rx.lo, rx.hi, ry.lo, ry.hi, fmtTickStrain, fmtTickLoad);
...
export function loadStrainChannelsSvg(points, title = "Sarcină – mărci tensiometrice (Ch6/Ch7/Ch8)") {
    ...
    const { grid, xLbl, yLbl, rangeLine, X, Y, innerH } = svgAxesAndTicks(rx.lo, rx.hi, ry.lo, ry.hi, fmtTickLoad, fmtTickStrain);
    const polyFor = (key) => {
        const pts = [];
        for (const p of dec) {
            const v = p[key];
...
function fmtTickStrainPercent(v) {
    if (!Number.isFinite(v))
        return "—";
    const a = Math.abs(v);
    if (a >= 1000 || (a > 0 && a < 1e-4))
...
export function soilStressStrainSvg(points, title = "σ – ε (principal)") {
    if (points.length < 2)
        return null;
    const dec = decimate(points, 800);
    ...
    const rx = axisDomainFromZero(Math.min(...xs), Math.max(...xs));
    const ry = axisDomainFromZero(Math.min(...ys), Math.max(...ys));
    const { grid, xLbl, yLbl, rangeLine, X, Y, innerH } = svgAxesAndTicks(rx.lo, rx.hi, ry.lo, ry.hi, fmtTickStrainPercent, fmtTickLoad);
...
export function soilStressVolStrainSvg(points, title = "σ – ε_V (aprox.)") {
    if (points.length < 2)
        return null;
    const dec = decimate(points, 800);
...
export function soilEpsilonDispTimeDualSvg(points, title = "ε și ΔH – timp") {
    if (points.length < 2)
        return null;
    const dec = decimate(points, 800);
    ...
    const X = (t) => PAD_L + (t - rt.lo) * sx;
    const Ye = (e) => PAD_T + (re.hi - e) * sye;
    const Yd = (d) => PAD_T + (rd.hi - d) * syd;
    ...
    const rangeLine = `<text x="${cx}" y="${H - 20}" text-anchor="middle" font-size="8" fill="#333" ${SVG_FONT}>${escXml(`t: ${fmtTickTime(rt.lo)} … ${fmtTickTime(rt.hi)} · ε: ${fmtTickStrainPercent(re.lo)} … ${fmtTickStrainPercent(re.hi)} · ΔH: ${fmtTickMm(rd.lo)} … ${fmtTickMm(rd.hi)}`)}</text>`;
    return `${svgOpen(title)}
  <rect x="0" y="0" width="${W}" height="${H}" fill="#fff"/>
  <text x="${W / 2}" y="17" text-anchor="middle" font-size="11" font-weight="600" fill="#0f3d3e">${escXml(title)}</text>
  ${legend}
...
export function soilMohrQuCuSvg(quKpa, cuKpa, title = "Cerc Mohr (q_u, c_u)") {
    if (!Number.isFinite(quKpa) || quKpa <= 0)
        return null;
    const sigma1 = quKpa;
    const sigma3 = 0;
    ...
    const maxT = Math.max(r * 1.12, cuOk ? cuKpa * 1.08 : 0, 1);
    const rx = axisRange(minS, maxS);
    const ry = axisDomainFromZero(0, maxT);
    const { grid, xLbl, yLbl, rangeLine, X, Y, innerH } = svgAxesAndTicks(rx.lo, rx.hi, ry.lo, ry.hi, fmtTickLoad, fmtTickLoad);
    const poly = arc.map((p) => `${X(p.sigma).toFixed(2)},${Y(p.tau).toFixed(2)}`).join(" ");
    let cuLine = "";
    if (cuOk && cuKpa <= maxT * 1.02) {
        const yc = Y(cuKpa);
...
export function triaxialMohrMultiSvg(circles, envelope, title = "Cercuri Mohr (τ–σ) — Triaxial") {
    const valid = (circles ?? []).filter((c) => Number.isFinite(c.sigma1Mpa) &&
    ...
    const { grid, xLbl, yLbl, rangeLine, X, Y, innerH } = svgAxesAndTicks(rx.lo, rx.hi, ry.lo, ry.hi, fmtTickStress, fmtTickStress);
    const envOk = envelope != null &&
...
export function timeLoadSvg(points, title = "Timp – sarcină", yAxisLabel = "F (kN)") {
    if (points.length < 2)
        return null;
    const dec = decimate(points, 800);
    ...
    const rx = axisDomainFromZero(Math.min(...xs), Math.max(...xs));
    const ry = axisDomainFromZero(Math.min(...ys), Math.max(...ys));
    const { grid, xLbl, yLbl, rangeLine, X, Y, innerH } = svgAxesAndTicks(rx.lo, rx.hi, ry.lo, ry.hi, fmtTickTime, fmtTickLoad);
...
export function stressTimeSvg(points, title = "Efort – timp", yAxisLabel = "σ (MPa)") {
    if (points.length < 2)
        return null;
    const dec = decimate(points, 800);
    const xs = dec.map((p) => p.t);
    ...
    const rx = axisDomainFromZero(Math.min(...xs), Math.max(...xs));
    const ry = axisDomainFromZero(Math.min(...ys), Math.max(...ys));
    const { grid, xLbl, yLbl, rangeLine, X, Y, innerH } = svgAxesAndTicks(rx.lo, rx.hi, ry.lo, ry.hi, fmtTickTime, fmtTickStress);
...
export function ucsBarSvg(valueMpa, title = "Rezistență la compresiune uniaxială (UCS)") {
    if (!Number.isFinite(valueMpa) || valueMpa < 0)
        return null;
...
export function stressMpaToLoadKn(stressMpa, diameterMm) {
