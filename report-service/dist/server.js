...
import { buildPointLoadPayload, buildUcsPayload, buildUnconfinedSoilPayload, buildYoungPayload, buildAbsorptionPorosityRockPayload, buildTriaxialRockPayload, buildPresiometryPayload, } from "./payload.js";
...
async function buildHtmlForTest(testId) {
    ...
    const templateFolder = testType === "absorption_porosity_rock"
    ...
    if (testType === "ucs") {
        ...
        payload = await buildPointLoadPayload(supabase, testId, cfg.templateCode, cfg.version, templatesRoot);
    }
    else if (testType === "unconfined_soil") {
        reportFileTag = "unconfined_soil";
        payload = await buildUnconfinedSoilPayload(supabase, testId, cfg.templateCode, cfg.version, cfg.sections);
    }
    else if (testType === "absorption_porosity_rock") {
        reportFileTag = "absorption_porosity_rock";
        payload = await buildAbsorptionPorosityRockPayload(supabase, testId, cfg.templateCode, cfg.version, cfg.sections);
    }
        ...
        throw new Error(`Tip test nesuportat pentru raport: ${display}. Permis: ucs, young, triaxial_rock, point_load, unconfined_soil, absorption_porosity_rock, presiometry. ` +
