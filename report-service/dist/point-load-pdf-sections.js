...
function resNum(rows, key) {
    return measNum(rows, key);
}
function pltTestKindLabel(v) {
    if (v == null)
        return "—";
...
function pltAnisotropyLabel(v) {
    if (v == null)
        return "—";
    const k = Math.floor(v);
    if (k === 0)
        return "Perpendicular pe foliație / șistozitate (T)";
    if (k === 1)
        return "Paralel cu foliație / șistozitate (//)";
    return String(v);
}
function depthInterval(df, dt) {
    if (df != null && dt != null)
        return `${df} – ${dt} m`;
    if (df != null)
        return `${df} m`;
...
export function buildPointLoadPdfSections(args) {
    ...
    let kUcs = null;
    if (kApplied != null && kApplied > 0)
        kUcs = kApplied;
    else if (kMeas != null && kMeas > 0)
        kUcs = kMeas;
    else if (ucsEst != null && Number.isFinite(ucsEst))
        kUcs = PLT_DEFAULT_UCS_CORRELATION_K;
    let areaDisp = "—";
    if (area != null)
        areaDisp = `${fmtSig3(area)} mm²`;
    ...
    const moistureCombined = [
        ...
        .filter((x) => x !== "—")
        .join("; ") || "—";
    const loadVsWeak = displayText(pl.loading_vs_weakness_note ?? undefined) !== "—"
    ...
    const equipParts = [];
    if (displayText(args.deviceDisplay) !== "—")
        equipParts.push(displayText(args.deviceDisplay));
    const tipMod = [displayText(pl.equipment_type), displayText(pl.equipment_model)]
        .filter((x) => x !== "—")
        .join(" ");
    if (tipMod)
        equipParts.push(tipMod);
    ...
    const wPct = pl.water_content_percent != null && Number.isFinite(pl.water_content_percent)
    ...
    const wManual = pl.water_content_percent != null && Number.isFinite(pl.water_content_percent)
    ...
    const rockClass = rockClassStored !== "—" ? rockClassStored : classifyIs50MpaStrengthRoOrDash(is50);
    const s8 = {
        num: "8",
